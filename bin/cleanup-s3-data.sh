...
if [ $# -lt 3 ]; then
    echo "Usage: $0 <ENTRYPOINT_URL> <BUCKET_NAME> <FILES_TO_KEEP> <PATH1> <PATH2> ..."
    exit 1
fi
...
for path in "${paths_to_check[@]}"; do
    ...
    files_to_delete=$(aws --output json --endpoint-url="${entrypoint_url}" s3api list-objects --bucket "${bucket_name}" --prefix "${path}" | \
    ...
    for file in $files_to_delete; do
        # Delete the file
        aws --endpoint-url="${entrypoint_url}" s3 rm "s3://${bucket_name}/${file}"
