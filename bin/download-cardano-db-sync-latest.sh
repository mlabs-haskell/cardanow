...
mkdir -p "${LOCAL_CARDANO_DB_SYNC_DATA_PER_SNAPSHOT}"
destination="${LOCAL_CARDANO_DB_SYNC_DATA_PER_SNAPSHOT}/cardano_db_sync_latest.tgz"
...
echo "Extracting $LOCAL_CARDANO_DB_SYNC_DATA_PER_SNAPSHOT/cardano_db_sync_latest.tgz to $LOCAL_CARDANO_DB_SYNC_DATA_PER_SNAPSHOT..."
if ! tar -xvf "${destination}" -C "${LOCAL_CARDANO_DB_SYNC_DATA_PER_SNAPSHOT}"; then
    echo "Failed to extract $LOCAL_CARDANO_DB_SYNC_DATA_PER_SNAPSHOT/cardano_db_sync_latest.tgz."
    exit 1
fi
